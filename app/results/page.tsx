...
export default function Results() {
  ...
  useEffect(() => {
    ...
    history.unshift({
      ...
      score: parsed.score,
      total: parsed.total,
      percent: Math.round((parsed.score / parsed.total) * 100),
      ...
      <div className="px-4">
        ...
        <div className="flex gap-3 mb-6">
          ...
          <button
            ...
            className="flex-1 bg-green-500 text-white py-3 rounded-2xl font-bold text-sm"
          >
            📲 Share
          </button>
          <a href="/" className="flex-1 bg-orange-500 text-white py-3 rounded-2xl font-bold text-center text-sm">
            🔄 Retry
          </a>
        </div>
            ...
            return (
              <div
                ...
                className={`bg-white rounded-2xl p-4 shadow-sm border-l-4 ${
                  isSkipped ? "border-gray-300" : isCorrect ? "border-green-500" : "border-red-500"
                }`}
              >
                <div className="flex justify-between items-center mb-2">
                  <p className="text-gray-400 text-xs font-medium">Question {i + 1}</p>
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                    isSkipped ? "bg-gray-100 text-gray-500" : isCorrect ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"
                  }`}>
                    {isSkipped ? "Skipped" : isCorrect ? "✓ Correct" : "✗ Wrong"}
                  </span>
                </div>
                <p className="text-gray-800 text-sm mb-3 leading-relaxed">{q.question}</p>
